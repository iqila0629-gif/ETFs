...
def main():
    ...
    add_normal(doc, "按策略逐条统计阈值加深后的效果方向，同时看平均回报与命中率两个相关系数；任一指标相关系数 < -0.5 即列入异常。全部 479 组可统计策略级阈值扫描中，负向策略平均回报中位数相关系数为 +0.79、命中率中位数为 +0.75；正向策略分别为 +0.48、+0.19。共 190 组异常策略（同一策略调整不同条件计为不同组）：")
