...
def main():
    ...
    add_normal(doc, "说明：偏移表示在原交付阈值基础上加减，并保留原条件的升/减方向。例如 EEM>0 的偏移 +1 表示 EEM>1%，QQQ<0 的偏移 -1 表示 QQQ<-1%；偏移 0 即交付阈值本身。对 EEM>0、JNK<0 这类条件，交付阈值就是 0%；对 XLK>=1%、VIX5d>=10% 等，交付阈值是 1%、10% 等原数值。某偏移下策略数偏少，是因为该偏移对应阈值下部分策略无法触发（有效交易数为 0 或数据不足），只有能触发且有数据的策略计入。")
