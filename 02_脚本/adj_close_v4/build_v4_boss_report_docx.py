...
def main():
    ...
    add_normal(doc, "按策略逐条统计阈值加深后的效果方向，判定改为“单调性偏离”：平均回报和命中率各自出现下降的次数即偏离点，任一指标偏离点 >=3 即列入异常。敏感度：偏离点 >=1 为 474 组、>=2 为 275 组、>=3 为 103 组、>=4 为 22 组、>=5 为 4 组；取 >=3 作为异常阈值，可过滤单个点的小波动。全部 563 组策略级阈值扫描中，最终保留 103 组异常。")
