...
def main():
    ...
    add_table(doc, ["阈值偏移", "平均回报中位数", "命中率中位数", "交易数中位数", "策略数"], [
        ...
        ["-1%", "0.308%", "55.0%", "994", "9"],
        ["-0.5%", "0.232%", "56.0%", "817", "33"],
        ["0%（交付阈值）", "0.407%", "56.9%", "519", "38"],
        ["+0.5%", "0.337%", "57.7%", "264", "98"],
        ["+1%", "0.379%", "58.1%", "136", "94"],
        ["+1.5%", "0.363%", "57.4%", "136", "66"],
        ["+2%", "0.365%", "56.0%", "139", "40"],
    ...
    add_table(doc, ["阈值偏移", "平均回报中位数", "命中率中位数", "交易数中位数", "策略数"], [
        ...
        ["0%（交付阈值）", "0.367%", "58.2%", "433", "18"],
        ["+0.5%", "0.323%", "56.9%", "301", "48"],
        ["+1%", "0.306%", "56.4%", "365", "42"],
        ["+1.5%", "0.336%", "56.3%", "383", "35"],
        ["+2%", "0.312%", "55.7%", "451", "28"],
    ], [Cm(2.6), Cm(3.0), Cm(3.0), Cm(3.0), Cm(2.0)], font_size=8)
    add_normal(doc, "结论：负向和正向策略在“交付阈值”处平均回报中位数最高，更松或更紧都不会明显改善；收紧后交易数普遍下降。")
    ...
    add_normal(doc, "按策略逐条统计阈值加深后的效果方向，同时看平均回报与命中率两个相关系数；任一指标相关系数 < -0.5 即列入异常。全部 140 组策略级阈值扫描中，负向策略平均回报中位数相关系数为 +0.69、命中率中位数为 +0.40；正向策略分别为 +0.20、+0.50。共 59 组异常策略（同一策略调整不同条件计为不同组）：")
    ...
    add_normal(doc, "异常策略数 >=10，逐策略阈值明细改放 CSV：v4_strategy_direction_detail.csv。改进措施：对这些异常策略不做“更深阈值”外推，优先保留交付阈值。")
