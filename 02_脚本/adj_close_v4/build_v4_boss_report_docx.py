...
def main():
    ...
    add_normal(doc, "本章假设：对单个策略，把某个条件阈值改深，例如 XLK<=-1% 且 该基金当日回报<=-2% 中的 XLK 改为 <=-2%/-2.5%/-3%，整个策略的平均回报和命中率应递增。以下全部按策略统计；多条件策略只调整被扫描的那个条件，其他子条件保持不变。4.1、4.2、4.3 使用同一份策略级阈值扫描结果。为什么只有 33 组：231 条策略中只有 32 条含可调阈值/连跌条件（31 条数值阈值 + 1 条连跌天数），其中 CYPIX 同时有两个可调阈值条件，因此共 33 组；其余 199 条是 >0/<0 方向条件，没有可调数值，不参与扫描。")
